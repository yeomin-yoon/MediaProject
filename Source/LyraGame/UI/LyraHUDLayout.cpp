...
void ULyraHUDLayout::NativeConstruct()
{
	Super::NativeConstruct();

	const FUIActionTag EscapeActionTag = FUIActionTag::TryConvert(TAG_UI_ACTION_ESCAPE);
	if (EscapeActionTag.IsValid())
	{
...
void ULyraHUDLayout::NativeOnInitialized()
{
	...
	if (EscapeActionTag.IsValid())
	{
		RegisterUIActionBinding(FBindUIActionArgs(EscapeActionTag, false,
		                                          FSimpleDelegate::CreateUObject(
			                                          this, &ThisClass::HandleEscapeAction)));
	}
...
void ULyraHUDLayout::HandleInputDeviceConnectionChanged(EInputDeviceConnectionState NewConnectionState,
...
void ULyraHUDLayout::DisplayControllerDisconnectedMenu_Implementation()
{
	...
	if (ControllerDisconnectedScreen)
	{
		...
		SpawnedControllerDisconnectScreen = UCommonUIExtensions::PushContentToLayer_ForPlayer(
			GetOwningLocalPlayer(), TAG_UI_LAYER_MENU, ControllerDisconnectedScreen);
	}
}
